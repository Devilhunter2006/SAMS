...
class AttendanceController extends Controller
{
    ...
    public function attendanceByDate(Request $request, $subjectId = null)
{
    $subjects = Subject::all();
    ...
    $students = Student::whereHas('subjects', function ($q) use ($currentSubject) {
        $q->where('subjects.id', $currentSubject->id);
    })->get();

    // Attendance records for chosen date
    $attendanceRecords = Attendance::where('subject_id', $currentSubject->id)
                                   ->whereDate('date', $date)
                                   ->pluck('status', 'student_id');

    return view('attendances.byDate', compact(
        'students',
        'subjects',
        'currentSubject',
        'attendanceRecords',
